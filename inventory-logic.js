...
// 4. Replacement for loadInventory (Fetching from Blockchain instead of Supabase)
async function loadInventoryFromChain() {
    try {
        ...
        for (let i = 1; i <= count; i++) {
            ...
            const row = `
                <tr>
                    ...
                    <td><span class="badge in-stock">Blockchain Verified</span></td>
                    <td>${new Date().toLocaleDateString()}</td>
                </tr>`;
            tbody.innerHTML += row;
        }
    } catch (err) {
        console.error("Failed to load inventory from blockchain:", err);
    }
}

// 5. Replacement for Checkout (Auto-signing the transaction)
async function processOrderOnChain() {
    const address = document.getElementById('deliveryAddress').value;
    const phone = document.getElementById('contactAddress').value;

    try {
        console.log("Sending transaction to testnet...");
        // This signs automatically using the PRIVATE_KEY above
        const tx = await contract.registerCustomer("Buyer", "User", address, BigInt(phone), "buyer@email.com");
        
        const receipt = await tx.wait();
        alert(`Order Recorded! Hash: ${receipt.hash}`);
...
window.onload = loadInventoryFromChain;