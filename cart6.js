...
function renderCartPage() {
    ...
    if (!display) return;
    if (cart.length === 0) {
        display.innerHTML = "<p style='text-align:center; color:#888;'>Cart is empty.</p>";
        if(totalsDisplay) totalsDisplay.innerHTML = "";
        return;
    }
    ...
    display.innerHTML = cart.map((item, index) => {
        ...
        return `
            <div class="cart-item" style="border-bottom:1px solid #eee; padding:15px; display:flex; gap:15px;">
                <img src="${item.image}" style="width:70px; border-radius:8px;">
                <div class="item-info" style="flex:1;">
                    <h4 style="margin:0;">${item.name}</h4>
                    <p style="font-size:0.75rem; color:#666; margin:4px 0;">Variants: ${item.variant}</p>
                    <p style="font-size:0.7rem; color:var(--accent-blue);">Delivery To: ${item.address}</p>
                    <p style="font-weight:bold;">Qty: ${item.quantity} | R${itemSubtotal.toLocaleString()}</p>
                </div>
    ...
    if(totalsDisplay) {
        totalsDisplay.innerHTML = `<h3>Subtotal (${totalQty} units): R${totalItemsPrice.toLocaleString()}</h3>`;
    }

// Auto-render on load if element exists
