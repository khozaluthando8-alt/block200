...
// ... existing RPC_URL and CONTRACT_ADDRESS configurations ...

/**
 * 1. Automatically updates the quantity based on the number of unit rows.
 * This replaces the manual quantity input requirement.
 */
function updateAutoQuantity() {
    const rows = document.querySelectorAll('.unit-row');
    const totalUnits = rows.length;
    
    // Update the UI display
    const qtyDisplay = document.getElementById('autoQtyDisplay');
    if (qtyDisplay) qtyDisplay.innerText = totalUnits;
    
    // Update the hidden input that the staging logic uses
    const qtyInput = document.getElementById('purchaseQty');
    if (qtyInput) qtyInput.value = totalUnits;
}

/**
 * 2. Adds a new variation row for the customer
 */
function addUnitRow() {
    const container = document.getElementById('unitRowsContainer');
    const firstRow = document.querySelector('.unit-row');
    ...
    // Reset selections in the new row
...
/**
 * 4. Modified stageOrder to loop through all dynamic rows
 */
function stageOrder() {
    const rows = document.querySelectorAll('.unit-row');
    const loc = document.getElementById('location').value; 
    ...
    // Validate that all rows have a selection
    rows.forEach(row => {
    ...
    if (!isValid) {
        alert("Please select both size and color for all units.");
        return;
    }
    ...
    if (typeof addToCart === 'function') {
        addToCart(orderItem);
        updateBadge();
        alert(`${qty} units added! Total units: ${qty}. Delivery to: ${loc}`);
    }
}

// Ensure listeners are attached when the page loads
window.addEventListener('load', () => {
    const addBtn = document.getElementById('addUnitBtn');
    if (addBtn) addBtn.addEventListener('click', addUnitRow);
